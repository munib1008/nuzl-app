...
class SalesPerformanceScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: const NuzlAppBar(title: 'Sales performance'),
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            Text('Your performance', style: t.titleSmall),
            ...
            Text('Team', style: t.titleSmall),
            const SizedBox(height: AppSpacing.x8),
            AsyncView<List<Map<String, dynamic>>>(
              value: team,
              onRetry: () => ref.invalidate(salesTeamProvider),
              data: (rows) => rows.isEmpty
                  ? Text('No team sales activity yet.',
...
class _MyScorecard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final stats = <({String label, String value, IconData icon, Color color})>[
      (label: 'Assigned leads', value: '${n('assigned')}', icon: Icons.inbox_outlined, color: AppColors.primary),
      (label: 'Quotes sent', value: '${n('quotes_sent')}', icon: Icons.request_quote_outlined, color: AppColors.accentGold),
      (label: 'Orders won', value: '${n('won')}', icon: Icons.emoji_events_outlined, color: AppColors.success),
      (label: 'Revenue', value: _money(num.tryParse('${m['revenue']}')), icon: Icons.payments_outlined, color: AppColors.success),
      (label: 'Conversion', value: '${n('conversion')}%', icon: Icons.trending_up, color: AppColors.primary),
      (label: 'Avg response', value: rh != null ? '${rh}h' : '—', icon: Icons.timer_outlined, color: AppColors.accentGold),
    ];
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${r['full_name'] ?? 'Member'}', style: t.titleSmall, maxLines: 1, overflow: TextOverflow.ellipsis),
              Text('${n('assigned')} leads · ${n('won')} won · ${n('conversion')}%',
